...
def setup_experiment(config):
    config.log_path = os.path.join(
        config.project_path, config.relative_log_path, config.exp_name
    )
...
def parse_config_name(config_names):
    ...
    config_parser = argparse.ArgumentParser(
        description="Runs an experiment given a specific config.",
        usage=f"main.py <config> [<args>] \n\nValid config names are:\n{valid_config_names}",
    )
    config_parser.add_argument(
        "config",
        help="Name of the config (default: 'default')",
        default="default",
        nargs="?",
    )
    ...
    for key, value in config.items():
        if type(value) == bool and value:
            parser.add_argument(
                f"--{key}",
                action="store_false",
                help="Boolean flag. Use without args to toggle. Default: True",
            )
        elif type(value) == bool and not value:
            parser.add_argument(
                f"--{key}",
                action="store_true",
                help="Boolean flag. Use without args to toggle. Default: False",
            )
        else:
            parser.add_argument(
                f"--{key}", default=value, type=type(value), help=f"default: {value}"
            )
    return parser
...
def load_wandb_files(run_path, exclude=None, include=None):
    ...
    for file in files:
        ...
        if exclude is not None and any(
            [re.match(pattern, file.name) for pattern in exclude]
        ):
            continue
        if include is not None and not any(
            [re.match(pattern, file.name) for pattern in include]
        ):
            continue
...
def load_wandb_checkpoint_and_config(run_path):
    ...
    if not os.path.exists(checkpoint_file):
        raise FileNotFoundError(
            f"Could not load checkpoitn file from wandb run {run_path}"
        )
