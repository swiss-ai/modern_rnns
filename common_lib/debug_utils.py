...
def check(tensor, shape):
    ...
    assert isinstance(shape, list) or isinstance(
        shape, tuple
    ), "shape arg has to be a tuple/list!"
    assert len(shape) == len(
        tensor_shape
    ), f"SHAPE GUARD: tensor shape {tensor_shape} not the same length as {shape}"
    ...
    for idx, (a, b) in enumerate(zip(tensor_shape, shape)):
        if b <= 0:
            ...
            assert (
                a == b
            ), f"SHAPE GUARD: at pos {str(idx)}, tensor shape {tensor_shape} does not match {shape}"
...
def log_global_stats_and_dist(tensors, prefix, log):
    ...
    global_mean = global_sum / global_numel if global_numel != 0 else 0
